...
Landed 2026-07-14: new-player guide (`docs/GUIDE.md`) covering the menus, HUD,
core loop, terrain rules, economy chains, mines/geologists, military, seafaring,
campaigns, saves and controls — each claim verified against the engine source or
in-game. Its screenshots are generated by `pnpm guide:shots`
(`e2e/guide-shots.ts` + `guide-shots.config.ts`, driving the app with the same
`__s2debug` helpers the e2e suite uses) into the git-ignored `docs/guide-shots/`,
never committed (original art, same policy as the converted assets).
